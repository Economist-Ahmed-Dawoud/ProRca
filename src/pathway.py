...
class ScmBuilder:
    ...
    def __init__(self, edges, nodes=None, visualize=False, viz_filename="dag_relationships", random_seed=0):
        self.edges = edges
        self.nodes = nodes
...
class CausalRootCauseAnalyzer:
    ...
    def _calculate_noise_contributions(self, df_agg: pd.DataFrame, 
                                anomaly_dates) -> Dict[str, np.ndarray]:
        """
        ...
        for node in self.scm.graph.nodes():
            try:
                ...
                print(f"Warning: Could not calculate noise contribution for {node}: {e}")
                noise_contributions[node] = np.array([0.0])  # Default value
                continue
                
        return noise_contributions

    def _calculate_structural_scores(self, df_agg: pd.DataFrame, 
                                  anomaly_dates) -> Dict[str, np.ndarray]:
        """
    ...
    def _find_root_cause_paths(self, start_node: str) -> List[List[Tuple[str, float]]]:
        ...
        all_paths = []
        visited = set()
        stack = [(start_node, [(start_node, self._calculate_combined_score(start_node))])]
        ...
        while stack:
            ...
            if not found_significant_parent and current_path[-1][1] >= self.min_score_threshold:
                all_paths.append(current_path)
        
        return all_paths
        
    def analyze(self, df_agg: pd.DataFrame, anomaly_dates, 
                start_node: str = 'PROFIT_MARGIN') -> Dict:
        """
        Main analysis method combining all approaches.
        """
        print("Calculating noise-based contributions...")
        self.noise_contributions = self._calculate_noise_contributions(df_agg, anomaly_dates)
    ...
    def analyze_by_date(self, df_agg: pd.DataFrame, anomaly_dates, start_node: str = 'PROFIT_MARGIN') -> Dict:
        """
        ...
        for ad in anomaly_dates:
            ...
            current_date = [ad]
            self.noise_contributions = self._calculate_noise_contributions(df_agg, current_date)
    ...
    def _calculate_path_significance(self, path: List[Tuple[str, float]], 
                                   root_node: str) -> float:
        """
    ...
    def _print_analysis_results(self, sorted_paths: List[Tuple[List[Tuple[str, float]], float]]):
        """
        ...
        for idx, (path, significance) in enumerate(paths):
            ...
            with dot.subgraph(name=f'cluster_{idx}') as c:
                ...
                for i, (node, combined_score) in enumerate(reversed_path):
                    ...
                    c.node(node, label=node_label, shape='box', style='rounded,filled', fillcolor='skyblue')
